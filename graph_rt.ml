  ...
  let (txt, nrow, ncol) =
    let tab =
      loop 1 where rec loop i =
        if i < String.length txt && txt.[i] = ' ' then loop (i + 1) else i
    in
    ...
    loop_i 0 0 0 0 where rec loop_i i nrow ccol ncol =
      if i = String.length txt then
        ...
        let (i, nrow, ccol, ncol) =
          ...
          else (i + 1, nrow, ccol + 1, ncol)
        in
        loop_i i nrow ccol ncol
      }
  in
  let sub_wid =
    rt_create_transient_widget wid "help mlbrot" (Some rt_destroy_widget)
...
value x_select_color xinfo count =
  ...
  else do { rt_select_color c; xinfo.last_col := c }
;
...
value refresh_pixmap_fast wid xinfo minfo rect_opt show_fast = do {
  ...
  (loop x y where rec loop i j =
     if j = y + h then if show_fast then () else rt_clear_widget wid
     else if i = x + w then do {
       if True || show_fast then rt_clear_area wid (x, j, w, 1) else ();
       loop x (j + 1)
     }
     else do { refresh_pixel drw minfo xinfo i j; loop (i + 1) j });
  if minfo.state.cur.exposing_state = ES_terminated then ()
  ...
  if Random.int 3 = 0 then
    ...
    if ord_ij = 0 then
      ...
      loop_imj min_imj min_imj 0 where rec loop_imj imj i j =
        if imj = max_imj + inc_imj then do { rt_clear_widget wid; None }
        else if i = w + h then do {
          if imj mod refresh_rate = 0 then do { rt_clear_widget wid; tempo ()
          }
          else ();
          loop_imj (imj + inc_imj) (imj + inc_imj) 0
        }
        else do {
          if i >= 0 && j >= 0 then refresh_pixel drw minfo xinfo i j else ();
      ...
      loop_ipj min_ipj 0 min_ipj where rec loop_ipj ipj i j =
        if ipj = max_ipj + inc_ipj then do { rt_clear_widget wid; None }
        else if j = 0 then do {
          if ipj mod refresh_rate = 0 then do { rt_clear_widget wid; tempo ()
          }
          else ();
    ...
    loop 0 min_i min_j where rec loop cnt i j =
      if ord_ij = 0 && j = max_j + inc_j || ord_ij = 1 && i = max_i + inc_i
      then do {
        rt_clear_widget wid;
        None
      }
      else if ord_ij = 0 && i = max_i + inc_i then do {
        if cnt = refresh_rate - 1 then do { rt_clear_widget wid; tempo () }
        else ();
...
value change_saturation_or_spreading xd wid minfo xinfo kev inc =
  ...
  else change_saturation xd wid minfo (fst xinfo) inc
;
...
value change_brightness_or_spreading xd wid minfo xinfo kev inc =
  ...
  else change_brightness xd wid minfo (fst xinfo) inc
;
...
value draw_internal_rectangle xd wid xinfo = do {
  ...
  rt_draw_arc drw
    (w / 2 - r - 1, h / 2 - r - 1, 2 * r + 2, 2 * r + 2, 0, 360 * 64)
};
...
value draw_iteration_points wid minfo xinfo x y w h = do {
  ...
  let fctx =
    {f_w = w; f_h = h; f_rot = rot; f_reduc = isc.reduc; f_xc = isc.xc;
     f_yc = isc.yc; f_nb_it = isc.nb_it; f_invert = minfo.invert;
     f_julia = isc.julia}
  in
  let fp x y = do {
  ...
  let restore_ctx =
    ...
    fun () → do { Mfloat.fp.val := None; Mfloat.M.set_num cur_num }
  in
...
value action_mandel2 wid info (xinfo, xfun) ev =
  ...
  [ RawEvKeyPress kev →
      ...
      | K_Ascii 'g' → do {
          ...
          let (x1, y2) = f 0 0 in
          let (x2, y1) = f w h in
          mprintf "x1 %s y1 %s x2 %s y2 %s\n" (Mfloat.Best.to_string x1)
            (Mfloat.Best.to_string y1) (Mfloat.Best.to_string x2)
            (Mfloat.Best.to_string y2);
          mprintf "dx %s dy %s\n"
            (Mfloat.Best.to_string (Mfloat.Best.sub x2 x1))
            (Mfloat.Best.to_string (Mfloat.Best.sub y2 y1));
          let d = abs_float (Mfloat.Best.to_float (Mfloat.Best.sub y2 y1)) in
          let (dman, dexp) =
            loop d 1.0 where rec loop d exp =
              if d < 1.0 then loop (d *. 10.0) (exp *. 10.0) else (d, exp)
          in
          mprintf "mant %g exp %g\n" dman dexp;
          mprintf "x1 %.15f x2 %.15f\n"
            (Mfloat.Best.to_float x1 -.
             mod_float (Mfloat.Best.to_float x1) (1. /. dexp))
            (Mfloat.Best.to_float x2 -.
             mod_float (Mfloat.Best.to_float x2) (1. /. dexp));
          mprintf "y1 %.15f y2 %.15f\n"
            (Mfloat.Best.to_float y1 -.
             mod_float (Mfloat.Best.to_float y1) (1. /. dexp))
            (Mfloat.Best.to_float y2 -.
             mod_float (Mfloat.Best.to_float y2) (1. /. dexp));
          let ix1 =
            Mfloat.Best.to_float x1 -.
            mod_float (Mfloat.Best.to_float x1) (1. /. dexp)
          in
      ...
      | K_Ascii 'm' → do {
          ...
          (loop 0 0 where rec loop i j =
             if i + 1 >= w then ()
               ...
               Array2dim.set ica (i / 2) (j / 2) (Array2dim.get old_ica i j);
      ...
      | K_Ascii ('r' | 'R' as g) → do {
          ...
          let pd =
            if g = 'R' then make_very_random_palette_def rs
            else
      ...
      | K_Ascii 's' →
          if xinfo.check_event then return_event xinfo ev
            ...
            if isc.exposing_state = ES_terminated then
              ...
              let di =
                (1. -. Info.zoom_coeff) *. float (w / 2) +. 0.5
              in
      ...
      | K_Ascii 't' → do { trace_slave_points stderr minfo isc w h; None }
      | K_Ascii 'u' →
          if xinfo.check_event then return_event xinfo ev
          else do {
      ...
      | IFDEF MPFR OR MPZ THEN K_Ascii '0' | K_KP_N 0 →
          (do {
      ...
      | IFDEF MPFR THEN K_Ascii '5' | K_KP_N 5 →
          (do { Info.set_num minfo isc N_mpf; None }) END
      | IFDEF MPZ THEN K_Ascii '6' | K_KP_N 6 →
          (do { Info.set_num minfo isc N_mpz; None }) END
      | K_Ascii '[' → do {
            ...
            let stat_event wid =
              ...
              [ RawEvExpose _ _ _ _ | RawEvConfigureNotify _ _ → do {
                  ...
                  if expose_cnt.val = 0 then do {
                    ...
                    in
                    let _ = mprintf "max x %d max y %d\n" max_nb_it max_y in
                    let min_x =
                      loop 0 where rec loop i =
                        if t.(i) < max_y / 100 then loop (i + 1) else i
                    in
                    let max_x =
                      loop max_nb_it where rec loop i =
                        if t.(i) < max_y / 100 then loop (i - 1) else i
                    in
                    let _ = mprintf "between %d and %d\n" min_x max_x in
                  ...
                  failwith
                    (sprintf "stat event %s\n"
                       (if Obj.is_int (Obj.repr ev) then
                        ...
                        else sprintf "(tag %d)" (Obj.tag (Obj.repr ev)))) ]
            in
  ...
  | RawEvConfigureNotify (ow, oh, _) (w, h, _) →
      if xinfo.check_event && (w <> ow || h <> oh) then return_event xinfo ev
...
Rt.term_font.(2) := "-*-terminus-medium-o-*-20-*";
...
value x_init init_pos init_wid init_hei c_pal_def c_pal = do {
  ...
  let xinfo =
    {xargs = xa; gen_widget = gen_wid; raw_widget = raw_wid; pixmap = pixmap;
     ...
     cursor_finish = rt_create_font_mouse xd xC_tcross; x_pal = x_pal;
     check_event = False; pending_event = None; no_main_loop = False}
  in
  let w = max init_wid scr_w in
  let h = max init_hei scr_h in
  (xinfo, w, h)
...
value update_point_in_pixmap xinfo chunk area (i, j) count = do {
  let drw = PixmapDr xinfo.pixmap in
  x_select_color xinfo count;
  if chunk = 1 then rt_draw_point drw (i, j)
  else
    let x = area.r_x in
    let y = area.r_y in
    let w = area.r_w in
...
value x_copy_area ginfo minfo (x, y, w, h) (x_dest, y_dest) =
...
value xfun =
  {update_point_in_pixmap = update_point_in_pixmap; g_copy_area = x_copy_area;
   ...
   g_set_palette = x_set_palette; g_unselect_cursor = x_unselect_cursor;
   g_widget_size = x_widget_size}
;

value ask_for_connection info i =
