(* $Id: i18n.ml,v 1.10 2018/02/05 16:37:28 deraugla Exp $ *)

value lex = "mlbrot.lexicon";

value lexicon = Hashtbl.create 1;
value lexicon_mtime = ref 0.0;
value lang = ref "";

value add_lexicon word transl = Hashtbl.add lexicon word transl;
...
value rec interp_slashes s =
  try
    ...
    if i + 1 < len then
      ...
      [ 'n' ->
          String.sub s 0 i ^ "\n" ^
          interp_slashes (String.sub s (i + 2) (len - i - 2))
      | _ -> s ]
    else s
  with
  [ Not_found -> s ]
      ...
      if len >= 4 && String.sub s 0 4 = "    " then
        ...
        if lang.val <> "" then
          loop "" where rec loop default =
            let t =
              try Some (input_lexicon_line ic) with [ End_of_file -> None ]
            in
            let ti =
              match t with
              [ Some t ->
            ...
            [ Some (t, i) ->
                ...
                then
                  ...
                  in
                  let t = interp_slashes (cut_trail_dot t) in
...
value gen_transl glang str = do {
  ...
  try Some (Hashtbl.find lexicon str) with [ Not_found -> None ]
};

value transl glang str =
  match gen_transl glang str with
  [ Some (Some str) -> str
  | Some None | None -> str ]
;
...
value fast_transl glang str =
  try
    match Hashtbl.find lexicon str with
    [ Some str -> str
    | None -> str ]
  with
...
value ptransl glang str =
  match gen_transl glang str with
  [ Some str_opt -> str_opt
  | None -> None ]
;
...
value ftransl glang (fmt : format 'a 'b 'c) =
  ...
  let tfmt =
    match gen_transl glang sfmt with
    [ Some (Some sfmt) -> sfmt
    | Some None | None -> sfmt ]
  in
  valid_format fmt tfmt
;
...
value rec eval_shift s =
  ...
  loop False 0 0 where rec loop changed i j =
    ...
    else if changed then eval_shift (Bytes.to_string (Bytes.sub t 0 j))
...
value etransl str : string =
  let (set, str) = eval_set str in
  let str = eval_app set str in
  Bytes.to_string (eval_shift str)
;
...
value ptransl str = ptransl lang.val str;
value transl str = transl lang.val str;
